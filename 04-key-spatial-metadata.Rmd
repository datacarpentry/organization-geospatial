...
* Geographical Coordinate System or Latitude and Longitude: degrees (which can
be expressed as decimal degrees, degrees: minutes: seconds, and more)
* Universal Transverse Mercator (UTM) coordinates: meters
...
* ellipsoid (or spheroid): a mathematically representation of the shape of the
earth. Visit
<a href="https://en.wikipedia.org/wiki/Earth_ellipsoid" target="_blank">Wikipedia's article on the earth ellipsoid </a> 
for more information
* geoid: a model of the earth's gravitatinal field which approximates the mean
sea level across the entire earth.  It is from this that elevation is measured.
Visit 
...
###Datum
To have a grid of x,y coordinates for the CRS, there must first be a defined 
point 0,0 from which all other coordinate values originate and a direction that
that the main axis extend from it. The datum describes which origin (0,0) and
axis orientation is chosen.  At a higher level, the datum describes the 
relationship between a specific ellipsoid and the center of the earth, while
giving the origin location for the CRS. 
...
used. Projected CRS have constant lengths, angles, and areas across the two
...
are based on a grid that wraps around the globe. The units are in degrees.  The
distance between the 2 degrees of longitude at the equator is ~ 69 miles, at 40N (or S) the distance is only 53 miles. 
...
Using the PROJ.4 syntax, it is possible to specify the complete set of parameters that define a particular SRS.
https://www.nceas.ucsb.edu/scicomp/recipes/projections
https://www.nceas.ucsb.edu/~frazier/RSpatialGuides/OverviewCoordinateReferenceSystems.pdf

 by its EPSG code (i.e.,
...
are some websites: http://www.epsg-registry.org/
