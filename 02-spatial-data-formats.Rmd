...
In this map of the Harvard Forest field site, there are many different features 
...
<figure>
    ...
    <figcaption>The map above contains point and line format vector data. 
...
shapefiles that were required to create this map? How do you know? 

</div>

```{r challenge-code-shapefile, echo=FALSE}
...
actually consists of a mandatory of 3 (or more) files:

* `.shp`: the file that contains the geometry for all features.
* `.shx`: the file that indexs the geometry.
* `.dbf`: the file that stores the attributes in a tabular format.

Those files all need to have the **same name** and to be stored in the same
...
## Shapefile Data Management - Sharing Vector Data
...
[*Introduction to Working With Vector Data in R* series]({{ site.baseurl }}/tutorial-series/vector-data-series/).
...
[*Convert from .csv to a Shapefile in R* tutorial]({{ site.baseurl }}/R/csv-to-shapefile-R/).
...
[*Raster Calculations in R - Subtract One Raster from Another and Extract Pixel Values For Defined Locations* tutorial]({{ site.baseurl }}/R/Raster-Calculations-In-R).
...
NEON Data Skills [tutorials focusing on HDF5]({{ site.baseurl }}/HDF5/). 
* **.grd:** An ESRI specific raster format.


## Storing Spatial Data On Your Computer

Sometimes it makes sense to store your data in a specific data directory 
(folder) that you use for one particular project. However, when you work with
spatial data on your computer, it is a good idea to store large data sets in a 
general data directory that you can easily access for many different projects. 
If you are working in a collaborative environment, use a shared data directory. 

<i class="fa fa-star"></i> **Data Tip:** Wondering why we’re saying 
**directory** instead of **folder**? See our discussion of 
[Directory vs. Folder]({{ site.baseurl }}/R/Set-Working-Directory#directory-vs-folder).
{: .notice}
