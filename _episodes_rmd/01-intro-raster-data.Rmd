...
some major features and types of raster data. A 
[later episode](({{ base.url }}/02-intro-vector-data/)) provides more detail about vector data.
...
Raster data is any pixelated (or grided) data where each pixel is associated
...
its coordinate reference system (or CRS, more on this in [a later episode](({{ base.url }}/03-crs/)).

![Raster Concept]("../images/dc-spatial-raster/raster_concept.png")
...
![USA landcover classification]("../images/USA_landcover_classification.png")

The map above shows the contiguous United States with landcover as
categorical data.
Each color is a different landcover category.  (Source: Homer, C.G., et
...
information. Photogrammetric Engineering and Remote Sensing, v. 81, no.
5, p. 345-354)
...
![Spatial extent image]("../images/dc-spatial-raster/spatial_extent.png")
...
![Resolution image]("../images/dc-spatial-raster/raster_resolution.png")
(Source: National Ecological Observatory Network (NEON))

## Raster Data Format for this Lesson
Raster data can come in many different formats. For this workshop, 
we will use the GeoTIFF format which has the extension `.tif`. 
A `.tif` file stores metadata
or attributes about the file as embedded `tif tags`. 
For instance, your camera might
store a tag that describes the make and model of the camera 
or the date the photo was taken when it saves a `.tif`. 
A GeoTIFF is a standard `.tif` image
format with additional spatial (georeferencing) information embedded in
the file as tags. These tags should include the following raster
metadata:

1. Extent
2. Resolution
3. Coordinate Reference System (CRS) - we will introduce this concept in [a later episode](({{ base.url }}/03-crs/))
3. Values that represent missing data (`NoDataValue`) - we will introduce this concept in [a later lesson](https://datacarpentry.org/r-raster-vector-geospatial/01-raster-structure/index.html).

We will discuss these attributes in more detail in [a later lesson](https://datacarpentry.org/r-raster-vector-geospatial/01-raster-structure/index.html).
In that lesson, we will also learn how to use R to extract raster
attributes from
a GeoTIFF file. 
...
![RGB multi-band raster image]("../images/dc-spatial-raster/RGBSTack_1.jpg")
(Source: National Ecological Observatory Network (NEON).)
